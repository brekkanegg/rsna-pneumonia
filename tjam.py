...
parser.add_argument('--from_superjjam', type=str2bool, default=True)
...
if pargs.is_train:
    ...
    if pargs.from_superjjam:
        save_dir = 'seed_{}_superjjam_tjam'.format(pargs.seed)

    else:
        save_dir = 'seed_{}_tjam'.format(pargs.seed)
    ...
    model.config.STEPS_PER_EPOCH = int(len(image_fps_train) / pargs.batch_size)
    config.VALIDATION_STEPS = 1  ## validation set have no more meaning
