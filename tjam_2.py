...
parser.add_argument('--rpn_roi_iou_pos_th', '--ioupth', type=float, default=0.5)
parser.add_argument('--rpn_roi_iou_neg_th', '--iounth', type=float, default=0.5)
...
parser.add_argument('--from_hem', type=str2bool, default=True)
...
### test phase 1
# train_png_dir = os.path.join(DATA_DIR, 'stage_2_train_images_woong')
...
image_fps_val = pneumonia_images
...
if pargs.is_train:
    ...
    if pargs.seed == '42':
        print('Using 42 weights')
        model_weight = './ckpt/seed_42/mask_rcnn_seed_42_0058.h5'
        save_dir = 'seed_42_test1_overfit'
    elif pargs.seed == '77':
        print('Using 77 weights')
        model_weight = './ckpt/seed_42/mask_rcnn_seed_77_0052.h5'
        save_dir = 'seed_77_test1_overfit'


    if pargs.from_jjam:
        save_dir = 'seed_{}_superjjam'.format(pargs.seed)
        ...
        # model_weight = "mask_rcnn_coco.h5"
    ...
    model.load_weights(model_weight, by_name=True, exclude=[
    ...
    model.train(dataset_train, dataset_val,
                learning_rate=config.LEARNING_RATE/10, epochs=pargs.max_epoch, layers='all', augmentation=None)  ## no need to augment yet
    ...
    history = pd.read_csv(
        ROOT_DIR + '/ckpt/' +  'seed_42_test1_overfit' + '/training_log.csv')
        # ROOT_DIR + '/ckpt/' + save_dir.replace('False', 'false') + '/training_log.csv')
    print('history: ', history)
