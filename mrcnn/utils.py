...
def resize_image(image, min_dim=None, max_dim=None, min_scale=None, mode="square"):
    ...
    #     w, h = image.shape[:2]
    #     sampled_images = []
    #     for fi1 in range(filter[0]):
    #         for fi2 in range(filter[1]):
    #             sampled_image_i = image[fi1::filter[0], fi2::filter[1]]
    #             sampled_images.append(sampled_image_i)
    #
    #     image = np.stack(sampled_images, axis=2)
    #     assert image.shape == (w / filter[0], h / filter[1], np.cumprod(filter)[-1])
...
# ## self.image_info[image_id]['orig_height'] Slicing
...
def polyphase_quadrature_filter(image, filter=[2,2], sample_num=3):
    w, h = image.shape
    ...
    sampled_image = np.stack(sampled_images, axis=2)

    assert sampled_image.shape == (w/filter[0], h/filter[1], np.cumprod(filter)[-1])

    sampled_image = sampled_image[:, :, :sample_num]
