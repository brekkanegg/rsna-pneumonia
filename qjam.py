...
parser.add_argument('--max_epoch', '--mep', type=int, default=7)
...
parser.add_argument('--superjam', '--sj', type=str2bool, default=False)
parser.add_argument('--qjam', '--qj', type=str2bool, default=True)


parser.add_argument('--image_size', '--ims', type=int, default=512)


parser.add_argument('--from_overfit', type=str2bool, default=True)
...
t1_png_dir = os.path.join(DATA_DIR, 'stage_1_test_images')
t1_fps, image_annotations = parse_dataset(t1_png_dir, anns=anns)

pneumonia_images = []
for img in tqdm(t1_fps):
...
# In[22]:
np.random.seed(42)  # 42
np.random.shuffle(sorted(pneumonia_images))


t1_fps = pneumonia_images
# image_fps_train = pneumonia_images

dataset_t1 = DetectorDataset(t1_fps, image_annotations, ORIG_SIZE, ORIG_SIZE)
dataset_t1.prepare()
...
if pargs.is_train:
    ...
    # load model
    if pargs.from_overfit:
        # save_dir = 'seed_{}_overfit'.format(pargs.seed)  ###
        save_dir = 'seed_{}_overfit_qjam'.format(pargs.seed)  ###
    ...
    if pargs.from_overfit:
        ...
        save_dir = 'seed_{}_qjam'.format(pargs.seed)
        # if pargs.superjam:
        #     save_dir = save_dir.replace('_jjam', '_superjjam')
    ...
    model.config.VALIDATION_STEPS = 1  ## validation set have no more meaning

    ## retrain focus on hard examples
    # print('Hard example mining cycle 1 start!')

    # model.train(dataset_jam, dataset_jam,
    #             learning_rate=3e-4, epochs=3, layers='all', augmentation=augmentation)

    # note: loss is no more a guide line, check the final map and recall

    model.config.STEPS_PER_EPOCH = int(len(t1_fps) / pargs.batch_size)
    model.train(dataset_t1, dataset_t1,
                learning_rate=1e-4, epochs=5, layers='heads', augmentation=augmentation)
...
if pargs.submit:
    ...
    result_df = predict(test_image_fps, filepath=submission_fp, min_conf=0.9) # 0.9
