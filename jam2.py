...
parser.add_argument('--max_epoch', '--mep', type=int, default=10)
...
print('val_images(=train images): ', len(val_dirs))
...
if pargs.is_train:
    ...
    if pargs.superjam:
        config.DEEPER_HEAD += 2  # add 2 more layer
    ...
    model.config.STEPS_PER_EPOCH = int(len(image_fps_val) / pargs.batch_size)
...
if pargs.submit:
    ...
    result_df = predict(test_image_fps, filepath=submission_fp, min_conf=0.9)
