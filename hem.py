...
parser.add_argument('--max_epoch', '--mep', type=int, default=3)
...
parser.add_argument('--weight_decay', '--wdc', type=float, default=3e-5)
...
if pargs.is_train:
    ...
    # current_pred_df = predict_temp(image_fps=image_fps_train[:10],
    #                                model_i=model_i,
    #                                config=config,
    #                                min_conf=0.9)
    ...
    if os.path.exists(current_pred_fp):
        ...
        current_pred_df = predict_temp(image_fps=image_fps_train,
                                       model_i=model_i,
                                       config=config,
                                       min_conf=0.9)
    ...
    model.train(dataset_train_hem, dataset_val,
                learning_rate=1e-4, epochs=pargs.max_epoch, layers='heads', augmentation=augmentation)
...
###################### cycle 2
    ...
    model_weight_hem1 = ROOT_DIR + '/ckpt/{}/mask_rcnn_{}_0003.h5'.format(save_dir, save_dir)
    ...
    model_i_2 = modellib.MaskRCNN(mode='inference',
                                config=inference_config,
                                model_dir=ROOT_DIR)

    model_i_2.load_weights(model_weight_hem1, by_name=True)

    current_pred_fp = os.path.join(ROOT_DIR, 'result_temp', save_dir + '_{}_{}_2.csv'.
                                   format(best_epoch, 0.9))

    current_pred_df = predict_temp(image_fps=current_hem_fps,
                                       model_i=model_i_2,
                                       config=config,
                                       min_conf=0.9)
    ...
    _, _, _, _, hresult_df2, fn_list = simple_evaluator_temp(
            gt_csv_path=DATA_DIR + '/stage_2_train_labels.csv',
            result_csv_path=current_pred_fp
        )

    current_hem = fn_list + \
                  list(hresult_df2[hresult_df2['result'] == 'FP2']['puid'])
                  # hresult_df[hresult_df['result'] == 'FP1'] +
    current_hem = list(set(current_hem))

    print('Hard examples in cycle 2: ', len(current_hem))
    ...
    ## retrain focus on hard examples
    print('Hard example mining cycle 2 start!')

    model.train(dataset_train_hem, dataset_val,
                learning_rate=3e-5, epochs=pargs.max_epoch*2, layers='all', augmentation=augmentation)
